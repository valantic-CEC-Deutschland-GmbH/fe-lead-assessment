...
Worked examples you can clone and adapt. **Planning:** move both repos under the **`shopware` GitHub org**, polish, and link them as **official** references from [developer.shopware.com/docs](https://developer.shopware.com/docs/) (see [Epic V1 — official samples](product-epic-backlog.md#epic-v1-before-scd-supported-shop-http-mcp) in the product backlog). Until then the canonical clones live under BrocksiNet:

- **In-process plugin:** [github.com/BrocksiNet/SwagMcpAdminUsers](https://github.com/BrocksiNet/SwagMcpAdminUsers) — MCP tool registered in PHP, ACL-aware admin user listing (matches the structure below).
- **Remote App:** [github.com/BrocksiNet/McpHelloWorld](https://github.com/BrocksiNet/McpHelloWorld) — minimal Shopware App demonstrating MCP via `Resources/mcp.xml` and signed webhooks.
...
- Tools must currently return a `string`. The MCP SDK wraps the return value into the protocol response automatically.
- This is a current implementation detail, not necessarily the final product shape. MCP tool results can also carry spec-native structured output (`structuredContent`, `isError`, `outputSchema`), so our response helper should be reviewed against the MCP spec before the API is treated as stable.
...
Unhandled exceptions in tool execution result in a generic MCP error (`-32603`). Extend `McpToolResponse` for structured error responses:
...
The abstract class provides `$this->success(array $data, array $meta = [])` and `$this->error(string $message)` which produce consistent JSON envelopes. It also includes a 100 KB response size guard and `requirePrivilege()` for ACL checks.
...
class ProductListTool extends McpToolResponse
{
