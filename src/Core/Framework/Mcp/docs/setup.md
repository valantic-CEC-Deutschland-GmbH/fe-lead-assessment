...
The MCP surface remains experimental until the next major release.
...
This is the **current POC configuration**.

**Product direction:** tool visibility should move to **per-integration selection in Admin**. New integrations should start with **no MCP tools selected**, and operators should explicitly enable only the tools they want for that integration. The global `shopware.mcp.allowed_tools` setting then becomes a coarse installation-wide safety switch, not the main product control.
...
4. Once Admin tool selection lands, enable only the MCP tools needed for that integration
