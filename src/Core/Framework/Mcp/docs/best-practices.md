...
### Consistent tool result shape

The current Shopware MCP PoC uses one internal JSON envelope:
...
That has been useful for the PoC because agents learn one pattern and apply it everywhere.

But this is a **Shopware-local convention**, not the MCP protocol itself. The MCP spec already defines tool results with:

- `content[]`
- optional `structuredContent`
- optional `isError`
- optional `_meta`
- optional `outputSchema` on the tool definition

So the product rule should be:

- keep results predictable for agents
- prefer spec-native tool result fields where the SDK supports them cleanly
- review the current `success/data/_meta` envelope against `structuredContent` and `outputSchema` before calling the response model final

If we keep the current helper for a while, document it as a temporary compatibility layer rather than the long-term protocol shape.
