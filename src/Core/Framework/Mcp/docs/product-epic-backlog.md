...
`shopware-context` already **duplicates** knowledge that also lives in `tools.md` / `security.md`. Every additional full prompt **multiplies drift risk** unless you **generate** prompt bodies from a single internal source or keep supplementary prompts short and link out to docs.
...
| ACL on data and outcome tools | **Partial** | Documented in `security.md`; some tools called out as no ACL (e.g. entity-schema); resources explicitly “no ACL” today | M | **Maybe** (tighten only if security review demands; AI helps tests) |
...
| Human-readable tool matrix for core vs merchant plugin | **Partial** | `tools.md` is the working source of truth for **primary user + target home + planned target name**. Keep it aligned with the ADR extraction and the merchant plugin plan. | S | **Pre-SCD** |
...
| Item | Status | Notes | Effort | May milestone |
|------|--------|-------|--------|---------------|
| Extension guide (plugins, bundles, apps) | **Partial** | Source: `extensibility.md`, `best-practices.md`, related; **destination:** developer.shopware.com | M (port + IA, not only copy-edit) | **Pre-SCD** (bounded chapter) / **Later** (exhaustive guide) |
...
| Security (HTTP MCP + integration + allowlist + dry-run) | **Partial** | Source: `security.md`; **destination:** public security chapter; local-tool expectations tie to Workstream 2 | M | **Pre-SCD** |
| Migration / companion story for `ai-coding-tools` | **Partial** | Public page: native `/api/_mcp` vs local Labs MCP | S–M | **Pre-SCD** |
| Editor-specific best practices | **Partial** | Source: `cursor-rule.md`; may stay partly “community” or live under guides | S | **Later** |
...
**Existing sample:** [BrocksiNet/SwagMcpAdminUsers](https://github.com/BrocksiNet/SwagMcpAdminUsers) — **pre-SCD [Epic V1](#epic-v1-before-scd-supported-shop-http-mcp):** **move** to **`shopware` org**, **polish**, align with `shopware.mcp.tool` / `#[McpTool]` conventions, update `extensibility.md` + public docs to **canonical `shopware/*` URL**. **Post-SCD:** second example or template extraction if still needed ([Epic V2](#epic-v2-after-scd-ecosystem-and-ux-depth)).

| Item | Status | Notes | Effort | May milestone |
|------|--------|-------|--------|---------------|
| In-process registration doc | **Partial** | `extensibility.md` + repo after **`shopware/*`** move | S | **Pre-SCD** |
...
*Related in-repo docs: [README.md](README.md) (index), [extensibility.md](extensibility.md) (includes example repo links), [agent-user-stories.md](agent-user-stories.md) (merchant scope vs ai-coding-tools).*
