...
- plugins use the `shopware.mcp.tool` DI tag, remapped by `McpToolCompilerPass`
...
- `McpToolCompilerPass` for mapping plugin tags (`shopware.mcp.tool` to `mcp.tool`) and enforcing unique names
...
- plugins tag services with `shopware.mcp.tool` (remapped by `McpToolCompilerPass`)
...
- `McpToolCompilerPass` for tag mapping, conflict detection, and allowlist enforcement
...
- the registration and discovery mechanism (`mcp.yaml`, DI tags, `McpToolCompilerPass`)
...
// McpToolCompilerPass -- tag mapping and conflict detection
...
// Plugin services.xml -- uses shopware.mcp.tool tag (remapped by compiler pass)
